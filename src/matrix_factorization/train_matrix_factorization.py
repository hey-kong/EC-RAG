...
from model import MODEL_IDS, embedding_dim
...
def train_loops(
    ...
    for epoch in range(num_epochs):
        ...
        if test_acc > best_test_acc:
            ...
            info.update({"best_test_acc": best_test_acc})
...
if __name__ == "__main__":
    ...
    json_path = "/data/dataset/balanced_mf_train_gpt4_dataset.jsonl"
    npy_path = "/data/dataset/balanced_mf_train_embedding.npy"
    save_path = "/data/models/mf_model.pth"
    ...
    with open(json_path, "r") as f:
        for line in f:
            item = json.loads(line)  # 解析单行 JSON
            item.pop("prompt", None)  # 删除 "prompt" 字段（如果存在）
            data.append(item)  # 添加处理后的数据
