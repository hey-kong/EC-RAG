...
rag_avg_time_match = re.search(r'rag_avg_time:\s*([\d.]+)', log_content)
average_f1_match = re.search(r'Average F1:\s*([\d.]+)', log_content)
cloud_api_cost_avg_match = re.search(r'cloud_api_cost_avg:\s*([\d.]+)', log_content)
...
if rag_avg_time_match and average_f1_match:
    ...
    if cloud_api_cost_avg_match:
        ...
        print(f"{filename}\t rag_avg_time: {rag_avg_time}, \taverage_f1: {average_f1}, \tcloud_api_cost_avg: {cloud_api_cost_avg}")
    else:
        print(f"{filename}\t rag_avg_time: {rag_avg_time}, \taverage_f1: {average_f1}")
else:
    print("未找到匹配的数据")
